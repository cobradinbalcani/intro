...
from produse.produse_sql import (listeaza_produs_flask_sql, adauga_un_produs_flask_sql,
                                 listeaza_toate_produsele_flask_sql, sterge_un_produs_flask_sql)

from comenzi.comenzi_sql import (listeaza_comanda_flask_sql, adauga_o_comanda_flask_sql,
                                 listeaza_toate_comenzile_flask_sql, sterge_o_comanda_flask_sql)
...
def get_product(id_produs):
    status, message = listeaza_produs_flask_sql(id_produs)
    return Response(status=status, response=json.dumps(message))


@app.route("/get_order/<string:id_comanda>", methods=["GET"])
def get_order(id_comanda):
    status, message = listeaza_comanda_flask_sql(id_comanda)
    return Response(status=status, response=json.dumps(message))
...
def put_product():
    ...
    status, message = adauga_un_produs_flask_sql(request_body)
    return Response(status=status, response=json.dumps(message))
...
def put_order():
    ...
    status, message = adauga_o_comanda_flask_sql(request_body)
    return Response(status=status, response=json.dumps(message))
...
def list_products():
    status, message = listeaza_toate_produsele_flask_sql()
    return Response(status=status, response=json.dumps(message))

@app.route("/list_orders", methods=["GET"])
def list_orders():
    status, message = listeaza_toate_comenzile_flask_sql()
    return Response(status=status, response=json.dumps(message))
...
def delete_product(id_produs):
    status, message = sterge_un_produs_flask_sql(id_produs)
    return Response(status=status, response=json.dumps({"message": message}))

@app.route("/delete_order/<string:id_comanda>", methods=["DELETE"])
def delete_order(id_comanda):
    status, message = sterge_o_comanda_flask_sql(id_comanda)
    return Response(status=status, response=json.dumps({"message": message}))
